...
# ==========================================
# 6. PAGE: ANALYTICS (ฉบับแก้ไขแบบ Super Robust)
# ==========================================
    ...
    if df_sales is None or df_sales.empty:
        st.info("📢 ไม่พบข้อมูลในชีต Sales กรุณาตรวจสอบการเชื่อมต่อ")
    else:
        try:
            # สร้างสำเนาเพื่อป้องกัน Error
            df_clean = df_sales.copy()
            
            # --- แก้จุดที่ 1: ล้างคอลัมน์ยอดเงิน (คอลัมน์ C / Index 2) ---
            # ทำให้แน่ใจว่าเป็นตัวเลข 100% ลบทุกอย่างที่ไม่ใช่ตัวเลขออก
            df_clean.iloc[:, 2] = pd.to_numeric(
                df_clean.iloc[:, 2].astype(str).str.replace(r'[^\d.]', '', regex=True), 
                errors='coerce'
            ).fillna(0)
            
            # --- แก้จุดที่ 2: ล้างคอลัมน์วันที่ (คอลัมน์ A / Index 0) ---
            df_clean.iloc[:, 0] = pd.to_datetime(
                df_clean.iloc[:, 0], 
                dayfirst=True, 
                errors='coerce'
            )
            
            # ตัดรายการที่วันที่เพี้ยนออก
            ...
            # --- คำนวณยอด ---
            today_total = df_clean[df_clean.iloc[:, 0].dt.date == today_date].iloc[:, 2].sum()
            week_total = df_clean[df_clean.iloc[:, 0] >= (now - timedelta(days=7))].iloc[:, 2].sum()
            month_total = df_clean[df_clean.iloc[:, 0].dt.month == now.month].iloc[:, 2].sum()
            ...
            st.dataframe(df_clean.sort_values(by=df_clean.columns[0], ascending=False), use_container_width=True)
            
        except Exception as e:
            st.error(f"⚠️ ระบบตรวจพบปัญหา: {e}")
            st.warning("คำแนะนำ: ลองตรวจสอบว่าใน Google Sheets หน้า Sales มีข้อมูลอย่างน้อย 1 แถวหรือยังครับ")
