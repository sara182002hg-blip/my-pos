...
# ✅ บังคับดึงข้อมูลจากหน้า Sales (gid=0) เท่านั้น
...
def generate_receipt_pdf(cart, total, method, bill_id):
    try:
        ...
        if method == "QR Code":
            ...
            pdf.image(qr_url, x=15, w=50) # ✅ ใส่ QR Code ลงในไฟล์ PDF
            
        return bytes(pdf.output(dest='S')) # ✅ ส่งออกแบบ Bytes เพื่อเลี่ยง Error
...
if menu == "🛒 ขายสินค้า (POS)":
    ...
    with col1:
        ...
        if not df_stock.empty:
            grid = st.columns(2) # ✅ ปรับเป็น 2 คอลัมน์เพื่อให้รูปใหญ่ขึ้นตามต้องการ
            for i, row in df_stock.iterrows():
                with grid[i % 2]:
                    st.markdown(f"""<div style="border:1px solid #444; padding:15px; border-radius:15px; text-align:center; margin-bottom:10px;">
                        <img src="{row['Image_URL']}" style="width:220px; height:220px; object-fit:contain;"><br>
                        <b style="font-size:1.4rem;">{row['Name']}</b><br>
                        <span style="color:#f1c40f; font-size:1.5rem;">{row['Price']:,} ฿</span><br>
    ...
    with col2:
        ...
        if st.session_state.cart:
            ...
            for name, item in list(st.session_state.cart.items()):
                ...
                if c_btn.button("❌ ลบ", key=f"del_{name}"):
    ...
    if not df_sales.empty:
        try:
            # ✅ ค้นหาคอลัมน์ "Total_Amount" ตามที่คุณตั้งใหม่
            if 'Total_Amount' in df_sales.columns:
                ...
                st.error("ไม่พบคอลัมน์ 'Total_Amount' ในชีต Sales")
                st.write("ชื่อคอลัมน์ที่พบ:", list(df_sales.columns))
