...
# --- 1. ตั้งค่าลิงก์ข้อมูล (ตรวจสอบว่า URL ถูกต้อง) ---
...
# ฟังก์ชันโหลดข้อมูลแบบป้องกันแคช เพื่อให้เรียลไทม์
def load_data(url):
    try:
        # ใส่ timestamp ต่อท้าย URL เพื่อบังคับให้โหลดใหม่ทุกครั้ง (Real-time)
...
if menu == "🛒 ขายสินค้า":
                ...
                with grid[i % 2]:
                    with st.container(border=True):
                        ...
                        if can_add:
                            ...
                            st.button("❌ สินค้าหมด", key=f"out_{i}", use_container_width=True, disabled=True)
        ...
        if st.session_state.receipt:
            ...
            with st.container(border=True):
                ...
                if r['method'] == "📱 PromptPay":
                    ...
                    qr_html = f'<div style="text-align: center; margin-top: 15px;"><img src="{qr_url}" width="250" style="border: 1px solid #ddd;"/><p style="font-size: 12px; color: #666; margin-top: 5px;">สแกนจ่ายเบอร์ {MY_PROMPTPAY}</p></div>'
                ...
                <div style="background-color: white; color: black; padding: 30px; border-radius: 10px; font-family: 'Courier New', Courier, monospace; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                    <div style="text-align: center; margin-bottom: 20px;"><h2 style="margin: 0;">TAS POS</h2><p style="font-size: 12px; color: #666;">ID: {r['id']}</p></div>
                    ...
                    <div style="display: flex; justify-content: space-between; font-size: 24px; font-weight: bold;"><span>รวมทั้งสิ้น</span><span>{r['total']:,} ฿</span></div>
                ...
                for name, item in list(st.session_state.cart.items()):
                    ...
                    if c3.button("➕", key=f"plus_{name}"):
                        ...
                        if item['qty'] < current_stock:
                            ...
                            st.toast(f"ขออภัย {name} ในสต็อกไม่พอ", icon="⚠️")
                        st.rerun()
                st.divider()
    ...
    if not df_sales.empty:
        ...
        st.success(f"รวมยอดขายทั้งหมด: {df_sales['ยอดรวม'].sum() if 'ยอดรวม' in df_sales.columns else 0:,} ฿")
    else:
        st.warning("ไม่พบข้อมูลยอดขายในระบบ")
