...
# ✅ บังคับใช้ gid=0 สำหรับหน้า Sales เพื่อแก้ปัญหาหาคอลัมน์ไม่เจอ
...
# ✅ ฟังก์ชัน PDF: เพิ่ม QR Code และแก้ไข Error การดาวน์โหลด
...
if menu == "🛒 ขายสินค้า (POS)":
    ...
    with col1:
        ...
        if not df_stock.empty:
            grid = st.columns(3) # ✅ ปรับกลับเป็น 3 คอลัมน์ให้เล็กลงและดูง่ายขึ้น
            for i, row in df_stock.iterrows():
                with grid[i % 3]:
                    st.markdown(f"""<div style="border:1px solid #444; padding:10px; border-radius:10px; text-align:center; margin-bottom:10px;">
                        ...
                        <b>{row['Name']}</b><br>
                        <span style="color:#f1c40f;">{row['Price']:,} ฿</span><br>
    ...
    with col2:
        ...
        if st.session_state.cart:
            ...
            if st.session_state.pdf_receipt:
                st.download_button("🖨️ ดาวน์โหลดใบเสร็จ", data=st.session_state.pdf_receipt, file_name=f"Bill_{int(time.time())}.pdf", use_container_width=True)
    ...
    if not df_sales.empty:
        try:
            # ✅ ค้นหาคอลัมน์ชื่อ 'ยอดรวม' หรือ 'Total_Amount'
            target_cols = [c for c in df_sales.columns if 'ยอดรวม' in c or 'Total_Amount' in c]
            if target_cols:
                ...
                st.error("ไม่พบคอลัมน์ 'Total_Amount' หรือ 'ยอดรวม' ในหน้า Sales")
                st.write("ชื่อคอลัมน์ที่พบในแผ่นนี้:", list(df_sales.columns))
